...
public class ecru_EntityAIPlantCacao extends EntityAIBase {
    ...
    public ecru_EntityAIPlantCacao(ecru_EntityMomiji pEntity, float pmoveSpeed, int bit) {
    ...
    public boolean func_75250_a() {
        if ((this.entityMomiji.getAttackMode() == 0 || !this.entityMomiji.attackModeHarvest) && this.entityMomiji.isTamed() && !this.entityMomiji.isSitting() && this.entityMomiji.isFreedom() && !this.entityMomiji.field_70128_L && !this.entityMomiji.func_70631_g_() && this.entityMomiji.getShears() != -1 && this.entityMomiji.getCacaoSeeds() != -1) {
    ...
    public void func_75249_e() {
        GameProfile gp = new GameProfile(UUID.randomUUID(), "momijiSowSeeds");
        FakePlayer fakePlayer = FakePlayerFactory.get(this.worldObject, gp);
        for (int i = 3; i < 21; i++) {
            if (this.entityMomiji.thisCacao(i)) {
                ItemStack is = this.entityMomiji.itemStacks[i];
                if (is.func_77973_b().func_77648_a(is, fakePlayer, this.worldObject, (int) this.xPosition, (int) this.yPosition, (int) this.zPosition, this.direction, 0.0f, 0.0f, 0.0f)) {
                    this.entityMomiji.func_71038_i();
                    if (this.entityMomiji.func_70301_a(i).field_77994_a == 0) {
                        this.entityMomiji.func_70298_a(i, 1);
                    }
                    return;
                }
            }
    ...
    private boolean getBlockList(World world, int i, int j, int k) {
        ...
        if (!bList.isEmpty()) {
            ...
            if (!bp2.beSeenCheck) {
                ...
                this.direction = bp2.direction;
                return true;
            }
            if (ecru_util.canEntityItemBeSeen(this.entityMomiji, bp2)) {
                this.xPosition = bp2.posX;
                this.yPosition = bp2.posY;
                this.zPosition = bp2.posZ;
                this.direction = bp2.direction;
    ...
    private int checkBlock(int x, int y, int z) {
        ...
        if (this.worldObject.func_147439_a(x, y, z) == Blocks.field_150364_r && (this.worldObject.func_72805_g(x, y, z) & 3) == 3) {
            if (this.worldObject.func_147439_a(x, y, z - 1) == Blocks.field_150350_a) {
                return 2;
            }
            if (this.worldObject.func_147439_a(x, y, z + 1) == Blocks.field_150350_a) {
                return 3;
            }
            if (this.worldObject.func_147439_a(x - 1, y, z) == Blocks.field_150350_a) {
                return 4;
            }
            if (this.worldObject.func_147439_a(x + 1, y, z) == Blocks.field_150350_a) {
                return 5;
