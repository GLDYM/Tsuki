...
public class ecru_BlockOreFlowerGold extends BlockContainer {
    ...
    public ecru_BlockOreFlowerGold() {
    ...
    public boolean func_149742_c(World world, int i, int j, int k) {
        if ((world.func_147439_a(i, j - 1, k) == mod_ecru_MapleTree.blockPlanter && world.func_72805_g(i, j - 1, k) != 0) || world.func_147439_a(i, j - 1, k) == mod_ecru_MapleTree.blockOreFlowerGold) {
            return true;
        }
        return false;
    }
    ...
    public void func_149690_a(World world, int i, int j, int k, int l, float m, int n) {
        ...
        switch (meta) {
            ...
            default:
                return;
            case 1:
                dp = this.random.nextInt(1) + this.blockInfo.dropOreNum[2][1];
                break;
            case 2:
                dp = this.random.nextInt(1) + this.blockInfo.dropOreNum[2][2];
                break;
            case 3:
                dp = this.random.nextInt(1) + this.blockInfo.dropOreNum[2][3];
                break;
            case 4:
                dp = this.random.nextInt(1) + this.blockInfo.dropOreNum[2][4];
                break;
            case 5:
                dp = this.random.nextInt(1) + this.blockInfo.dropOreNum[2][5];
                break;
            case 6:
                dp = this.random.nextInt(1) + this.blockInfo.dropOreNum[2][6];
                break;
            case 7:
                dp = this.random.nextInt(1) + this.blockInfo.dropOreNum[2][7];
                break;
        }
        ecru_IdList ecru_idlist = this.blockInfo;
        func_149642_a(world, i, j, k, new ItemStack(ecru_IdList.dropItemId[2], dp, 0));
    }

    public void func_149734_b(World world, int i, int j, int k, Random par5Random) {
    }

    public ecru_TileEntityOreFlower func_149915_a(World world, int mata) {
    ...
    public void func_149651_a(IIconRegister par1IconRegister) {
        ...
        for (int i2 = 0; i2 < 8; i2++) {
            tx_flower[i2] = par1IconRegister.func_94245_a("MapleTree:gold_flower_" + i2);
        }
    }
