...
public class PepperCropBlock extends BaseCropBlock {
    ...
    public PepperCropBlock(Properties properties, Supplier<? extends ItemLike> seeds) {
    ...
    private boolean canHarvest(BlockState state, Player player) {
        return !player.isShiftKeyDown() && state.getValue(getAgeProperty()) >= getMaxAge() - 1;
    }
    ...
    private void harvest(Level level, BlockPos pos) {
        ...
        TsukiNormalItemSet pepperType = age >= getMaxAge()
                ? TsukiNormalItemSet.PEPPERCORN_RED
                : TsukiNormalItemSet.PEPPERCORN_GREEN;
        popResource(level, pos, new ItemStack(ItemRegistry.MATERIALS.get(pepperType).get()));
