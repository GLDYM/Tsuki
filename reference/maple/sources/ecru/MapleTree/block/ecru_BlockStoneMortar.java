...
public class ecru_BlockStoneMortar extends BlockContainer {
    private final Random random;
    private IIcon tx_body;

    public ecru_BlockStoneMortar() {
    ...
    public int func_149645_b() {
        return mod_ecru_MapleTree.renderStoneMortarID;
    }

    public ecru_TileEntityStoneMortar func_149915_a(World var1, int meta) {
        return new ecru_TileEntityStoneMortar();
    }
    ...
    public boolean func_149727_a(World world, int i, int j, int k, EntityPlayer par5EntityPlayer, int par6, float par7, float par8, float par9) {
        if (!world.field_72995_K) {
            par5EntityPlayer.openGui(mod_ecru_MapleTree.instance, mod_ecru_MapleTree.guiIdStoneMortarId, world, i, j, k);
            return true;
        }
        return true;
    }
    ...
    public void func_149749_a(World par1World, int par2, int par3, int par4, Block par5, int par6) {
        ecru_TileEntityStoneMortar tileentitychest = (ecru_TileEntityStoneMortar) par1World.func_147438_o(par2, par3, par4);
        if (tileentitychest != null && par1World.func_147439_a(tileentitychest.getGearPosX(), tileentitychest.getGearPosY(), tileentitychest.getGearPosZ()) == mod_ecru_MapleTree.blockPowerShaftGear) {
            ecru_TileEntityPowerShaftGear tile = (ecru_TileEntityPowerShaftGear) par1World.func_147438_o(tileentitychest.getGearPosX(), tileentitychest.getGearPosY(), tileentitychest.getGearPosZ());
            String key = getStr(par2) + getStr(par3) + getStr(par4);
            tile.deleteMap(key);
        }
